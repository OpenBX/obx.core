 ...
 * | * Исправлена ошибка в логике генерации файла (updater.dep.php) для обновлений
 * | * Радикально увеличена скорость работу --raw-lang-check
 * |   Так же для этого методо обработана ситуация с многострочным комментариемв одной строке
 *
 * [1.1.4]
 * === Сборщик ===
 * * Учетено поведение Битрикса при загрузке выпуска модуля.
 *   Срипты обновления подмодулей будут ликвидированы из модуля.
 * * Добавлена страница настроек модуля.
 * * Метод OBX\Core\Tools::setLessProductionReady теперь используется для явного задания поведения LESS-файлов.
 *   Если данный метод не вызван ни разу, то будет использована опция из настроек модуля
 * * Доработано семество классов Settings, упрощающие создание файла options.php для модулей
 *
 * [1.1.5]
 * === Tools ===
 * * Исправлена ошибка в методе \OBX\Core\Tools::rusQuantity()
 *   Использование вида echo "25 ".\OBX\Core\Tools::rusQuantity(25, "день", "дня", "дней"); возвращало нулевой результат
