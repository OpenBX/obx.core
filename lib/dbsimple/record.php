...
class Record extends MessagePoolDecorator {
	...
	public function __get($field) {
		if(!array_key_exists($field, $this->fieldsValues)) {
			...
			// TODO: lazyLoad
			// „то бы получить недостающие данные, нам необходим первичный или unique ключ
			// Ќадо об€зательно проверить получение этих данных в методе readFromDBResult
	 ...
	 * @return bool
	 * @throws Err
	 * ѕрочитать запись по значени€м полей уникального индекса
	 * »м€ индекса можно указать €вно. Ёто необходимо в том случае если в $fields
	 * указаны значени€ полей подход€щих под несколько уникальных ключей и нужна €вна€
	 * возможность убрать неопределенность. ќднако если не указать, то будет выбран первый
	 * попавшийс€ ключ, значени€ которого будут заполнены
	 */
	public function readByUniqueIndex(array $fields, $indexName = null) {
		// ѕрвоер€ем есть ли пол€ уникального индекса
		if(!$this->_checkUniqueIndex($fields, $indexName)) {
			$e = new Err('', Err::E_READ_BY_UQ_NOT_ALL_FLD);
			$this->addErrorException($e);
			// «десь об€зательно кидаем исключение,
			// поскольку задача программиста проследить за тем, что бы были заполнены все пол€ unique-индекса
			throw $e;
		}
	...
	/**
	 * ѕровер€ет заполнены ли все пол€ указанного unique-индекса
	 * @param array &$fields
